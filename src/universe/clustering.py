...
def perform_clustering(etfs, min_samples=2, xi=0.05, min_cluster_size=0.1):
    ...
    scaled_distance = scaler.fit_transform(distance_matrix)
    
    # 2. PCAで可視化のために次元削減（2次元に）
    pca = PCA(n_components=2)
    pca_result = pca.fit_transform(scaled_distance)
    ...
    cluster_labels = optics.fit_predict(scaled_distance)
